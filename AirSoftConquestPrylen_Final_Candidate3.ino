...
Known Quirks
Disconnect 5v when uploading otherwise it will fail with an error code. 
...
//Short GPIO 18 with GND and restart for debug mode.
bool skipSetup = false; //Bypass setup for debugging
...
ButtonDebounce redButton(redButtonPin, 50);  //Button, debounce with INPUT_PULLUP
ButtonDebounce blueButton(blueButtonPin, 50);
ButtonDebounce yellowButton(yellowButtonPin, 50);
ButtonDebounce greenButton(greenButtonPin, 50);
int buttonRepeatDelay = 150;

//Extra debounce option to avoid insta clicking through setup flow
bool blockEnterButton = false;
...
void setup() {
  ...
  splashScreen(); //Show splash screen on display
  startupLEDsequence();
